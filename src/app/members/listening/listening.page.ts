...
export class ListeningPage implements OnInit {
  ...
  constructor(
    ...
    this.route.queryParams.subscribe(params => {
      ...
      if (getNav.extras.state.file) {
        this.title = getNav.extras.state.file.title;
        this.filename = getNav.extras.state.file.file_name;
        this.filepath = getNav.extras.state.file.file_path;
            ...
            .catch(async err => {
              ...
              if (err.code == 1) {
                ...
                let loadingEl = await this.loadingCtrl.create({
                  message: 'Aguarde enquanto baixamos os arquivos do nosso servidor...'
                });
                loadingEl.present();
                ...
                fileTransfer
                  .download(
                    url, 
                    this.storageDirectory + this.filename,
                    true,
                    {
                      headers: {
                        "Authorization": "Basic c2VlZGU3NTI6NEUyZEo2RnE="
                      }
                    })
                  .then(entry => {
