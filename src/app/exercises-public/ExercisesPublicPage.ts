...
@Component({
  ...
  templateUrl: './exercises-public.page.html',
  styleUrls: ['./exercises-public.page.scss'],
})
export class ExercisesPublicPage implements OnInit {
  showExercise = false;
  loading = true;
  noExercises = false;
  exercises: any;
  ...
  wins = 0;
  loses = 0;
  constructor(private http: HttpClient, private server: ServerService, private alertCtrl: AlertController) { }
  ngOnInit() {
  }
  init() {
    this.score = 0;
  ...
  getByLevel(level) {
    ...
    this.http.post(this.server.protocol + this.server.host + this.server.port + "/exercises/getByLevel", {
      level: level
    }, httpOptions)
      .pipe(map(res => res)).subscribe(response => {
